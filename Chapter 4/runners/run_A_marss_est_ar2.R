# ============================================================================
# run_A_marss_est_ar2.R
#
# In-sample fit del modelo MARSS-hybrid con RHO ESTIMADO Y second-order.
# Una sola corrida, prior Beta(alpha_rho, beta_rho), mean del growth rate
# usa lags t-1 y t-2 con coefs fijos (phi1, phi2) = (2, -1) [match a MARSS].
# Llamada: Rscript run_A_marss_est_ar2.R [ALPHA_RHO] [BETA_RHO] [WARMUP] [SAMPLING]
#   ej. Rscript run_A_marss_est_ar2.R                # defaults: Beta(2,2)
#   ej. Rscript run_A_marss_est_ar2.R 2 2 2000 8000
#
# Stan: stan/renewal_A_marss_est_ar2.stan (use_filter=1, sigma_obs=MARSS Stage 1)
# Output: outputs/stan_fits_marss_rhoest_ar2/rho_estimated.rds
# ============================================================================
...
cat(sprintf("\n=== IN-SAMPLE MARSS second-order (rho ESTIMATED, Beta(%g, %g)) ===\n",
            ALPHA_RHO, BETA_RHO))
cat("AR order: second-order on lambda with fixed (phi1, phi2) = (2, -1)\n")
...
fits_dir <- file.path(proj_root, "outputs", "stan_fits_marss_rhoest_ar2")
dir.create(fits_dir, showWarnings = FALSE, recursive = TRUE)
stan_path <- file.path(proj_root, "stan", "renewal_A_marss_est_ar2.stan")
...
save_object <- list(
  ...
  model_variant = "MARSS_hybrid_rhoest_ar2",
  draws = draws_arr,
...
# v2 (TMPDIR + cp + sync) fallo: el cp termino con exit code != 0 sin
# mensaje de error visible (.err vacio), R hizo stop() pero stderr no
# flusheo. Demasiados pasos vulnerables. v3 simplifica:
#   1. gc() para liberar fit MCMC (~5-10GB) antes del write
#   2. saveRDS DIRECTO a Lustre (sin tmp, sin cp)
#   3. system("sync") para forzar Lustre OST flush
#   4. Sys.sleep(30) margen amplio
#   5. Verificar tamano > 50MB + readRDS sanity
