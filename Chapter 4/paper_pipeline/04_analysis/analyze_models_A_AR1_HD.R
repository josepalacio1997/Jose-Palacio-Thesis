# =============================================================================
# analyze_models_A_AR1_HD.R
# -----------------------------------------------------------------------------
# HAUSDORFF ABLATION version of analyze_models_A_AR1.R
# Reads the Hausdorff ablation fits from:
#   Modelos AR con rho est. y rho=0/fits_HD/
# and writes outputs to results_modelA_AR1_HD/ to keep them separate
# from the primary PEHD analysis.
#
# Expected fits (4 first-order variants, Hausdorff W):
...
# ============================================================================
FITS_DIR <- file.path(PROJ_DIR, "Modelos AR con rho est. y rho=0", "fits_HD")
FIT_PATHS <- list(
...
if (length(crps_results) > 0) {
  ...
  write.csv(crps_overall, file.path(OUT_DIR, "crps_summary_AR1_HD.csv"),
            row.names = FALSE)
  ...
  write.csv(crps_by_plant, file.path(OUT_DIR, "crps_by_plant_AR1_HD.csv"),
            row.names = FALSE)
  ...
  p_fig14 <- ggplot(crps_overall_long,
    ...
    theme(plot.title = element_text(face = "bold", color = RICE_BLUE_DARK),
          plot.subtitle = element_text(color = NEUTRAL_GRAY),
          strip.text = element_text(face = "bold"))
  ggsave(file.path(OUT_DIR, "fig14_crps_summary_AR1_HD.pdf"),
  ...
  p_fig15 <- ggplot(crps_by_plant_plot,
    ...
    theme(plot.title = element_text(face = "bold", color = RICE_BLUE_DARK),
          ...
          legend.position = "bottom")
  ggsave(file.path(OUT_DIR, "fig15_crps_by_plant_AR1_HD.pdf"),
...
cat("  fig15_crps_by_plant_AR1_HD.pdf\n")
...
cat("  crps_summary_AR1_HD.csv / crps_by_plant_AR1_HD.csv\n")
