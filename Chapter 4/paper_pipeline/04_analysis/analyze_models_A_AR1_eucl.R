# =============================================================================
# analyze_models_A_AR1_eucl.R
# -----------------------------------------------------------------------------
# EUCLIDEAN ABLATION version of analyze_models_A_AR1.R
# Reads the Euclidean ablation fits from:
#   Modelos AR con rho est. y rho=0/fits_eucl/
# and writes outputs to results_modelA_AR1_eucl/ to keep them separate
# from the primary PEHD analysis.
#
# Expected fits (4 first-order variants, Euclidean W):
...
# ============================================================================
FITS_DIR <- file.path(PROJ_DIR, "Modelos AR con rho est. y rho=0", "fits_eucl")
FIT_PATHS <- list(
...
make_multipanel_pdf <- function(long_df, varname, file_out, y_lab) {
  ...
  for (lab in names(fits)) {
    ...
    base_plot <- make_paper_panel(
      sub, COLORS_4[[lab]], title = fit_tag, y_lab = y_lab,
...
if (length(crps_results) > 0) {
  ...
  write.csv(crps_overall, file.path(OUT_DIR, "crps_summary_AR1_eucl.csv"),
            row.names = FALSE)
  ...
  write.csv(crps_by_plant, file.path(OUT_DIR, "crps_by_plant_AR1_eucl.csv"),
            row.names = FALSE)
  ...
  p_fig14 <- ggplot(crps_overall_long,
    ...
    theme(plot.title = element_text(face = "bold", color = RICE_BLUE_DARK),
          plot.subtitle = element_text(color = NEUTRAL_GRAY),
          strip.text = element_text(face = "bold"))
  ggsave(file.path(OUT_DIR, "fig14_crps_summary_AR1_eucl.pdf"),
  ...
  p_fig15 <- ggplot(crps_by_plant_plot,
    ...
    theme(plot.title = element_text(face = "bold", color = RICE_BLUE_DARK),
          ...
          legend.position = "bottom")
  ggsave(file.path(OUT_DIR, "fig15_crps_by_plant_AR1_eucl.pdf"),
...
cat("  fig15_crps_by_plant_AR1_eucl.pdf\n")
...
cat("  crps_summary_AR1_eucl.csv / crps_by_plant_AR1_eucl.csv\n")
