# ============================================================================
# run_A_marss_est_eucl.R
#
# EUCLIDEAN ABLATION variant of run_A_marss_est.R
# -----------------------------------------------------------------------------
# Identical to run_A_marss_est.R except the spatial weight matrix W is loaded
# from W_eucl.rds (centroid-to-centroid Euclidean distance + RBF kernel +
# row-normalization), in place of the PEHD-derived W_pop50 used in the
# primary fit. All other model components, MARSS Stage-1 inputs, priors, HMC
# settings, and Stan files remain unchanged.
#
# Prerequisite: run paper_pipeline/01_W_matrix/compute_W_matrices.R first
# to generate ../results/W_eucl.rds (and W_PEHD.rds for reference).
#
# In-sample fit del modelo MARSS-hybrid (Ensor-style) con RHO ESTIMADO.
# Llamada: Rscript run_A_marss_est_eucl.R [ALPHA_RHO] [BETA_RHO] [WARMUP] [SAMPLING]
#   ej. Rscript run_A_marss_est_eucl.R                  # defaults: Beta(2,2)
#
# Stan:   stan/renewal_A_marss_est.stan          (unchanged)
# Output: outputs/stan_fits_marss_rhoest_eucl/rho_estimated.rds
# ============================================================================
...
# --- EUCLIDEAN ABLATION: load precomputed W_eucl.rds ------------------------
...
# precomputed Euclidean spatial weight matrix from disk. To regenerate it,
# run paper_pipeline/01_W_matrix/compute_W_matrices.R.
W_eucl_path <- file.path(proj_root, "Modelos AR con rho est. y rho=0",
                         "paper_pipeline", "results", "W_eucl.rds")
stopifnot(file.exists(W_eucl_path))
W_eucl <- readRDS(W_eucl_path)
W_geo  <- W_eucl[Batch_focus, Batch_focus, drop = FALSE]
cat(sprintf("Loaded W_eucl from %s\n", W_eucl_path))
cat(sprintf("W_eucl dim: %d x %d, row-sums OK: %s\n",
...
fits_dir <- file.path(proj_root, "outputs", "stan_fits_marss_rhoest_eucl")
